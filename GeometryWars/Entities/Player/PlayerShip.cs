...
namespace GeometryWars.Entities.Player
{
    class PlayerShip : Entity
	{
		...
		int _cooldowmRemaining = 0;

		int _framesUntilRespawn = 0;
		...
		public override void Update()
		{
			...
			if (aim.LengthSquared() > 0 && _cooldowmRemaining <= 0)
			{
				...
				var randomSpread = _rand.NextFloat(-0.04f, 0.04f) + _rand.NextFloat(-0.04f, 0.04f);
		...
		private void MakeExhaustFire()
		{
			if (Velocity.LengthSquared() > 0.1f)
			{
				...
				var velMid = baseVel + _rand.NextVector2(0, 1);
				...
				var vel1 = baseVel + perpVel + _rand.NextVector2(0, 0.3f);
				var vel2 = baseVel - perpVel + _rand.NextVector2(0, 0.3f);
				GeoWarsGame.ParticleManager.CreateParticle(TextureLoader.LineParticle, pos, Color.White * alpha, 60f, new Vector2(0.5f, 1),
		...
		public void Kill()
		{
            ...
            Parallel.For(0, 1200, 
                i => {
                        var speed = 18f * (1f - 1 / _rand.NextFloat(1f, 10f));
                        var color = Color.Lerp(Color.White, explosionColor, _rand.NextFloat(0, 1));
